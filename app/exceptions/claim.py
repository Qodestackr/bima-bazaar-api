...
class ClaimNotFoundException(KnownError):
    def __init__(self, policy_id: Optional[int] = None):
        message = f"Claim with id {policy_id} not found" if policy_id else "Claim not found"
        super().__init__(
            message=message,
            status_code=404,
...
class ClaimValidationError(KnownError):
...
class ClaimBusinessRuleError(KnownError):
